...
testing::AssertionResult verifyAllocation(const std::string &path_to_graph,
  ...
  for (const auto &v : variables) {
    if (mapping.find(v) == mapping.end()) {
      return testing::AssertionFailure()
             << "Variable " << v << " did not get mapped to a register!";
    }

    if (mapping.at(v) < 1 || mapping.at(v) > num_registers)
      return testing::AssertionFailure()
             << "Variable " << v << " mapped to register " << mapping.at(v)
             << " which is out of range [" << 1 << "," << num_registers << "]";
  }

  for (const auto &interference : interferences) {
    if (mapping.at(interference.first) == mapping.at(interference.second))
      return testing::AssertionFailure()
             << "Variables " << interference.first << " and "
             << interference.second << " were mapped to the same register: "
             << mapping.at(interference.first);
  }
  ...
  for (const auto &e : mapping) unique_registers.insert(e.second);
